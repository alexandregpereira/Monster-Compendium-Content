#!/usr/bin/env python3
"""
google_translate_conditions.py — Translate a conditions JSON file using the Google Translate API.

Can be run standalone or imported as a module by other scripts.

Usage (standalone):
    python3 scripts/google_translate_conditions.py <input_json> --lang <lang_code> [options]

Examples:
    python3 scripts/google_translate_conditions.py json/en-us/conditions.json --lang es
    python3 scripts/google_translate_conditions.py json/en-us/conditions.json --lang pt-br --output json/pt-br/conditions.json

Only the values of "name" and "description" fields are translated.
All JSON keys are kept in English. The "index" and "type" fields are never translated.

Formatting rules after translation:
  - "name"        → title case  (e.g. "Cegado")
...
def title_case(text: str) -> str:
    ...
    Example: "cegado por magia" → "Cegado por Magia"
    """
    text = text.strip()
    if not text:
        return text
...
def derive_output_path(input_path: str, lang: str) -> str:
    ...
    Example:
        json/en-us/conditions.json  →  json/es/conditions.json
...
def extract_texts(data: list) -> tuple[list[str], list[tuple[int, str]]]:
    """
    Walk the conditions data and collect every translatable string value.

    Returns:
        texts — flat list of source strings ready to send to the translation API
        refs  — parallel list of (condition_idx, field_name) tuples so each
                translated string can be written back to the right location in data
    """
    texts = []
    refs = []

    for condition_idx, condition in enumerate(data):
        for field in ("name", "description"):
            if field in condition:
                texts.append(condition[field])
                refs.append((condition_idx, field))
...
def apply_translations(data: list, refs: list[tuple[int, str]], translated: list[str]) -> None:
    ...
    apply field-specific formatting:
      - "name"        → title_case()   (e.g. "Cegado")
      - "description" → ensure_upper() (first character uppercased only)
    """
    for (condition_idx, field), value in zip(refs, translated):
        if field == "name":
            formatted = title_case(value)
        else:
            formatted = ensure_upper(value)
        data[condition_idx][field] = formatted


# ---------------------------------------------------------------------------
# Public entry point (used by translate_conditions.py and callable from other scripts)
# ---------------------------------------------------------------------------

def translate_conditions_file(
    input_path: str,
    lang: str,
    api_key: str,
    ...
    Returns the path of the saved output file so callers (e.g. translate_conditions.py)
    ...
    print(f"  Found {len(texts)} translatable strings across {len(data)} conditions.")
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Translate a conditions JSON file using the Google Translate API."
    )
    parser.add_argument(
        "input",
        help="Path to the source conditions JSON file (e.g. json/en-us/conditions.json)",
    )
    parser.add_argument(
        "--lang", "-l",
        required=True,
